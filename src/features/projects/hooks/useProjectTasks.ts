...
export function useProjectTasks(projectId: string) {
  ...
  async function updateTask(
    taskId: string,
    updates: { title?: string; notes?: string | null; status?: TaskStatus; priority?: 'low' | 'medium' | 'high' | null; start_date?: string | null; due_date?: string | null; recurring?: 'never' | 'daily' | 'weekly' | 'monthly' | 'yearly' | null; list_id?: string }
  ) {
    const prev = tasks
